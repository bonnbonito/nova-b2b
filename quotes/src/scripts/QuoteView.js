...
export default function QuoteView() {
  ...
  const addToCart = () => {
    ...
    formData.append(
      'product',
      NovaAccount.product_name ? decodeHTML(NovaAccount.product_name) : 'Custom Project'
    );
  ...
  const finalPrice = currency === 'USD' ? quotePrice : quotePrice * EXCHANGE_RATE;

  const { standard } = shippingRates(finalPrice, currency);
          ...
          <div id="quoteDiv" ref={quoteRef} className="pb-6">
            <div className="flex gap-2 items-center mb-4 ">
              <h6 className="m-0 text-nova-primary">
                STATUS: <span className="text-sm">{NovaAccount?.quote_status?.label}</span>
              </h6>
            </div>
            <div className="mb-10 block">
              <h4 className="uppercase">QUOTE ID: Q-{NovaAccount?.ID.padStart(4, '0')}</h4>
            </div>
            ...
            {signage?.map(item => (
              <PricesView key={item.id} id={item.id} item={item}></PricesView>
            ))}
            ...
            <div className="flex justify-between gap-4">
              <h5>
                {`${
                  NovaAccount?.quote_status?.value === 'ready' ? 'SUBTOTAL' : 'ESTIMATED SUBTOTAL'
                }`}
                :
              </h5>{' '}
              <h5>
                {finalPrice > 0
                  ? `${currency}$${Number(finalPrice.toFixed(2)).toLocaleString()}`
                  : 'TBD'}
              </h5>
            </div>
            ...
            <div className={`text-sm mb-2 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
              The shipping cost depends on the address in your account. You can change the shipping
              type and shipping address during checkout.
            </div>
            ...
            <div className="flex justify-between gap-4 border-b pb-14 mt-8">
              ...
              <h4>
                {estimatedTotal > 0
                  ? `${currency}$${Number(parseFloat(estimatedTotal).toFixed(2)).toLocaleString()}`
                  : 'TBD'}
              </h4>
            </div>

            <p className="mt-4 text-[10px] text-[#5E5E5E]">
              Freight charges may vary based on factors such as shipping destination, package size,
              and delivery speed. 
            </p>
          </div>
          {isOwner && (
