...
function ModalSave({ action, btnClass, label, storage }) {
  const { signage, tempFolder, missing, isLoading, setIsLoading, partner } = useAppContext();
  ...
  return (
    <Dialog.Root open={submitting ? true : open} onOpenChange={setOpen}>
      {isLoading ? (
        <button className={btnClass}>{isLoading ? 'Please wait...' : label}</button>
      ) : (
      ...
      <Dialog.Portal>
        ...
        {!error ? (
          !submitted ? (
              ...
              <form ref={formRef} onSubmit={handleFormSubmit}>
                ...
                {action === 'processing' || action === 'update-processing' ? (
                  ...
                  <p className="text-sm mt-4">
                    ...
                    {action === 'update' ? 'UPDATE QUOTE' : 'SAVE TO DRAFT'}. Go to MOCKUPS and
                    select DRAFTS.
                  </p>
                )}
            ...
            <Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[550px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none z-[51] overflow-auto">
              <Dialog.Description className="mt-[10px] mb-5 text-[15px] leading-normal">
                ...
                {action === 'draft' && (
                  <span dangerouslySetInnerHTML={{ __html: saveDraftMessageHtml() }}></span>
                )}
                {(action === 'update' || action === 'update-processing-admin') && (
                  <span dangerouslySetInnerHTML={{ __html: updateMessageHtml() }}></span>
                )}
                <span className="gap-2 block justify-center sm:flex">
                  {(action === 'draft' ||
                    action === 'update' ||
          ...
          <Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[550px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none z-[51] overflow-auto">
            ...
            <Dialog.Description className="mt-[10px] mb-5 text-[15px] leading-normal">
              {error.type === 'missing' && (
                <span className="font-title mb-4 uppercase">Missing Values:</span>
              )}
            </Dialog.Description>
            <div
