...
import { colorOptions, vinlyl3635Options } from '../../../../utils/ColorOptions';

import ColorsDropdown from '../../../../utils/ColorsDropdown';

import VinylColors from '../../../../utils/VinylColors';

import { acrylicChannelThicknessOptions, acrylicFrontOptions } from '../../options';
...
export function Letters({ item }) {
  ...
  const [waterproof, setWaterproof] = useState(item.trimLessWaterproof ?? INDOOR_NOT_WATERPROOF);
  ...
  const [acrylicFront, setAcrylicFront] = useState(item.acrylicFront ?? 'White');
  ...
  function updateSignage() {
    ...
    setSignage(prevSignage =>
      prevSignage.map(sign => (sign.id === item.id ? { ...sign, ...updateDetails } : sign))
    );
  }
  ...
  useOutsideClick(
    [colorRef, fontRef, acrylicFront === '3M 3630 Vinyl' ? vinyl3MRef : null].filter(Boolean),
    () => {
      if (acrylicFront === '3M 3630 Vinyl') {
      ...
      <div className="quote-grid mb-6">
        ...
        <Dropdown
          ...
          options={acrylicFrontOptions.map(option => (
            <option key={option.option} value={option.option} defaultValue={option == acrylicFront}>
              {option.option}
            </option>
          ))}
          value={acrylicFront}
