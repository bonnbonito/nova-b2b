...
export function Letters({ item }) {
  ...
  const [stainLessMetalFinish, setStainLessMetalFinish] = useState(item.stainLessMetalFinish);
  const [stainlessSteelPolished, setStainlessSteelPolished] = useState(item.stainlessSteelPolished);

  const [color, setColor] = useState(item.color ?? {});
  const [openColor, setOpenColor] = useState(false);
  const [waterproof, setWaterproof] = useState(item.waterproof ?? '');
  const [selectedThickness, setSelectedThickness] = useState(item.metalThickness);
  ...
  const [metalMountingOptions, setMetalMountingOptions] = useState(metalInstallationOptions);
  ...
  const handleOnChangeThickness = e => {
    const target = e.target.value;
    const selected = metalThicknessOptions.filter(option => option.value === target);
  ...
  const computePricing = () => {
    ...
    if (waterproof) tempTotal *= waterproof === INDOOR_NOT_WATERPROOF ? 1 : 1.02;
  ...
  useEffect(() => {
    ...
    if (selectedThickness?.value === '4') {
      ...
      newMountingOptions = metalInstallationOptions.filter(
        option => option.option !== STUD_MOUNT && option.option !== STUD_WITH_SPACER
      );
    } else {
      ...
      <div className="quote-grid mb-6">
        ...
        <Dropdown
          ...
          options={metalOptions.map(metal => (
            <option key={metal.option} value={metal.option} defaultValue={metal.option === metal}>
              {metal.option}
            </option>
          ))}
          value={metal}
