...
import { anodizedPricing, calculateLetterPrice, spacerPricing } from '../../../../utils/Pricing';
...
export function Letters({ item }) {
  ...
  const [selectedFinishing, setSelectedFinishing] = useState(item.aluminumFinishing ?? '');

  const [anodizedFinishing, setAnodizedFinishing] = useState(item.anodizedFinishing ?? '');
