...
export function Letters({ item }) {
  ...
  const [returnColor, setReturnColor] = useState(item.returnColor ?? { name: '', color: '' });
  ...
  const handleOnChangeThickness = e => {
    const target = e.target.value;
    const selected = fabricatedThicknessOptions.filter(option => option.value === target);
  ...
  const computePricing = () => {
    ...
    if (waterproof) tempTotal *= waterproof === INDOOR_NOT_WATERPROOF ? 1 : 1.01;
  ...
  return (
    <>
      ...
      <div className="quote-grid">
        {(color?.name == 'Custom Color' || returnColor?.name == 'Custom Color') && (
