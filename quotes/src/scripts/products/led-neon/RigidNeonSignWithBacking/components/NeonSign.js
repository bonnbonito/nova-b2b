...
import { EXCHANGE_RATE, INDOOR_NOT_WATERPROOF } from '../../../../utils/defaults';
...
export const NeonSign = ({ item }) => {
  ...
  const [rigidBacking, setRigidBacking] = useState(item.rigidBacking ?? 'Frosted Clear PC');
  const [wireExitLocation, setWireExitLocation] = useState(item.wireExitLocation ?? 'Bottom Right');
  const [wireType, setWireType] = useState(item.wireType ?? '6ft Clear DC5521 female');
