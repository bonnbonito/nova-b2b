...
import { arrayRange, spacerStandoffDefaultOptions } from '../../../../utils/SignageOptions';
...
import { ledSpacerStandoffDefaultOptions, remoteControlOptions } from '../../neonSignOptions';
...
export const NeonSign = ({ item }) => {
  ...
  const [neonLength8mm, setNeonLength8mm] = useState(item.neonLength8mm ?? 'TBC');
  const [rigidM4StudLength, setRigidM4StudLength] = useState(item.rigidM4StudLength ?? '');
  const [neonLength10mm, setNeonLength10mm] = useState(item.neonLength10mm ?? 'TBC');
  const [neonLength14mm, setNeonLength14mm] = useState(item.neonLength14mm ?? 'TBC');
  const [neonLength20mm, setNeonLength20mm] = useState(item.neonLength20mm ?? 'TBC');
  ...
  const [remoteControl, setRemoteControl] = useState(item.remoteControl ?? 'No');
  ...
  const computePricing = () => {
    ...
    if (waterproof) {
      tempTotal *= waterproof === INDOOR_NOT_WATERPROOF || waterproof === 'N/A' ? 1 : 1.3;
    }
  ...
  const handleOnChangeWaterproof = e => {
    ...
    setWireType(target === INDOOR_NOT_WATERPROOF ? '6ft open wires' : '10ft open wires');
    setWireTypeOptions(
