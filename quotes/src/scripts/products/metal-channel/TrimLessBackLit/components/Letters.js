...
export function Letters({ item }) {
  ...
  const [color, setColor] = useState(item.backLitfaceColor ?? { name: '', color: '' });

  const [sameColor, setSameColor] = useState(item.backLitSameColor ?? { name: '', color: '' });
  ...
  const [returnCustomColor, setReturnCustomColor] = useState(item.returnCustomColor ?? '');

  const [selectedFinishing, setSelectedFinishing] = useState(item.backLitFinishing);

  const [metalFinish, setMetalFinish] = useState(item.backLitMetalFinish ?? '');
  const [faceReturnColor, setFaceReturnColor] = useState(item.backLitfaceReturnColor ?? '');
  ...
  const checkAndAddMissingFields = () => {
    ...
    if (selectedFinishing === 'Painted') {
      ...
      if ((color?.name === 'Custom Color' || sameColor?.name === 'Custom Color') && !customColor) {
        missingFields.push('Add the Pantone color code of your custom color.');
      }
      if (returnColor?.name === 'Custom Color' && !returnCustomColor) {
        missingFields.push('Add the Pantone color code of your return custom color.');
      }
    }
  ...
  useEffect(() => {
    if (depth?.value && letterPricingTables) {
      const table = convert_json(getLetterPricingTableByTitle(depth?.depth, letterPricingTables));
  ...
  return (
    <>
      ...
      <div className="quote-grid mb-6">
          ...
          <>
            <Dropdown
              ...
              options={returnColorOptions.map(option => (
                <option key={option} value={option} defaultValue={option === faceReturnColor}>
                  {option}
                </option>
              ))}
        ...
        <Dropdown
          ...
          options={acrylicRevealOptions.map(option => (
            <option key={option} value={option} defaultValue={option == acrylicReveal}>
              {option}
            </option>
          ))}
          value={acrylicReveal}
      ...
      <div className="quote-grid">
        {(color?.name == 'Custom Color' || sameColor?.name == 'Custom Color') && (
