...
import { colorOptions, vinlyl3635Options, whiteOptionsResin } from '../../../../utils/ColorOptions';
...
const smallPunctuations = parseFloat(
  NovaQuote.small_punctuations_pricing ? NovaQuote.small_punctuations_pricing : 1
);
...
export function Letters({ item }) {
  ...
  const [color, setColor] = useState(item.returnColor ?? { name: 'Black', color: '#000000' });
  ...
  const [acrylicReveal, setAcrylicReveal] = useState(item.acrylicReveal ?? '1/5"');
  ...
  const [frontBackVinyl, setFrontBackVinyl] = useState(item.frontBackVinyl ?? '');

  const [frontAcrylicCover, setFrontAcrylicCover] = useState(item.frontAcrylicCover ?? 'White');

  const [lightingOptions, setLightingOptions] = useState(lightingPackagedOptions);

  const [includedItems, setIncludedItems] = useState('');

  const [mounting, setMounting] = useState(item.mounting ?? '');

  const [ledLightColor, setLedLightColor] = useState(item.ledLightColor ?? '6500K White');
  ...
  useEffect(() => {
    async function fetchLetterPricing() {
      try {
        const response = await fetch(NovaQuote.letters_pricing_api + item.product);
  ...
  useEffect(() => {
    ...
    if (waterproof !== INDOOR_NOT_WATERPROOF) {
      ...
      setDepthOptions(prev => {
        if (JSON.stringify(prev) !== JSON.stringify(aluminumResinDepthOptions)) {
  ...
  }, [waterproof, lettersHeight, depth, selectedLetterHeight, letterHeightOptions]);
    ...
    setLetterHeightOptions(() =>
      Array.from(
        ...
        (_, index) => {
          ...
          return (
            <option key={index} value={val} defaultValue={val === selectedLetterHeight}>
              {val}"
            </option>
          );
        }
  ...
  useEffect(() => {
    if (depth?.value) {
      const newHeightOptions = letterPricing?.filter(item => {
        ...
        return (
          value !== '' && value !== null && value !== undefined && value !== false && !isNaN(value)
        );
      });
  ...
  useEffect(() => {
    if (waterproof) {
      if (waterproof === INDOOR_NOT_WATERPROOF) {
        ...
        setLightingOptions(
          lightingPackagedOptions.filter(option => option.value === LIGHTING_INDOOR)
        );
      } else {
  ...
  return (
    <>
      ...
      <div className="py-4">
        <label className="uppercase font-title text-sm tracking-[1.4px] px-2">Text</label>
        <input
      ...
      <div className="quote-grid mb-6">
        ...
        {(frontAcrylicCover === '3M 3630 Vinyl' || frontAcrylicCover === '3M 3635 Vinyl') && (
          <>
            <Dropdown
        ...
        <Dropdown
          ...
          options={ledLightColors.map(color => (
            <option key={color} value={color} defaultValue={color == ledLightColor}>
              {color}
            </option>
          ))}
          value={ledLightColor}
      ...
      <div className="text-xs text-[#9F9F9F] pt-4">
        We size the letters in proportion to your chosen font. Some uppercase/lowercase letters may
        appear shorter or taller than your selected height on the form to maintain visual harmony.
      </div>
    </>
  );
}
