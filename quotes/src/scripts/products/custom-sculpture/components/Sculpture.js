...
export function Sculpture({ item }) {
  ...
  const [projectTimeline, setProjectTimeline] = useState(item.projectTimeline ?? '');
    ...
    <>
      ...
      <div className="quote-grid mb-6">
        <Dropdown
          ...
          options={materialOptions.map(material => (
            <option key={material} value={material} defaultValue={material === sculptureMaterial}>
              {material}
            </option>
          ))}
      ...
      <div className="quote-grid">
        <div className="px-[1px] col-span-4">
          <label className="uppercase font-title text-sm tracking-[1.4px] px-2">DESCRIPTION</label>
          <textarea
