...
export default function LayeredAcrylic() {
    ...
    switch (item.component) {
      case 'AcryLic':
        if (item.type === 'letters') {
          output = (
            <AcrylicLetters key={item.id} index={index} item={item} productId={item.product} />
          );
        } else {
          output = <AcrylicLogo key={item.id} index={index} item={item} productId={item.product} />;
        }
        break;
      case 'AcrylicUV':
        output = <AcrylicUV key={item.id} index={index} item={item} productId={item.product} />;
        break;
      case 'AcrylicMetalLaminate':
        if (item.type === 'letters') {
          output = (
            <AcrylicMetalLetters key={item.id} index={index} item={item} productId={item.product} />
          );
        } else {
          output = (
            <AcrylicMetalLogo key={item.id} index={index} item={item} productId={item.product} />
          );
        }
        break;
    }
  ...
  const signageOutput = signage.map((item, index) => {
    return (
      <Signage key={item.id} index={index} id={item.id} item={item} editable={false}>
          ...
          <Note title="Note">
            <ul className="text-sm">
              <li className="font-bold">
                Layer 1 is the bottom layer. Each subsequent layer (Layer 2, Layer 3, etc.) is
                positioned above the previous one
              </li>
              <li>The minimum stroke for 3M double-sided tape is 10mm.</li>
              <li>
                For stud pins: The minimum stroke is 12mm (1/2”) and the minimum acrylic thickness
                is 1/4" (6mm).
              </li>
              <li>
                You can choose a thicker acrylic to accommodate the design. If you choose thinner
                acrylic, take note that it cannot use stud pins and the sign must be carefully glued
              ...
              {openLayer && (
                <div className=" border-black p-4 rounded-md border shadow-2xl">
                  {productLayers.map((layer, index) => {
                    return <ProductLayer key={index} layer={layer} length={signage.length} />;
                  })}
                </div>
              )}
            </div>
