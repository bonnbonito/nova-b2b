function display_stainless_metal_pricing_table() {
	console.info('DOM loaded');

	const adminSignageOptions = AdminStainlessMetal.pricing_table;
	...
	if (metalPricing.length > 0) {
		const headers = ['Height', '4', '6', '9', '12'];

		const metalTable = createTable(metalPricing, headers);

		document
			.getElementById('laserCutStainlessLetterTable')
			.appendChild(metalTable);
	}

	const logoThicknesses = [4, 6, 9, 12];
	...
	logoThicknesses.forEach((thickness) => {
		...
		if (table) {
			...
			document
				.getElementById(`laserCutStainlessLogo-${thickness}mm`)
...
if (document.readyState === 'loading') {
	...
	document.addEventListener(
		'DOMContentLoaded',
		display_stainless_metal_pricing_table
	);
} else {
