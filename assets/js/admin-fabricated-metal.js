function display_fabricated_metal_pricing_table() {
	console.info('DOM loaded');

	const adminSignageOptions = AdminFabricatedMetal.pricing_table;
	...
	if (metalPricing.length > 0) {
		const headers = ['Height', '1', '1.5', '2', '3', '4', '5', '6'];

		const metalTable = createTable(metalPricing, headers);

		document
			.getElementById('fabricatedMetalLetterTable')
			.appendChild(metalTable);
	}

	const logoThicknesses = [8, 10, 15, 20, 30, 40, 50];
	...
	logoThicknesses.forEach((thickness) => {
		...
		if (table) {
			...
			document
				.getElementById(`fabricatedStainlessMetal-${thickness}mm`)
				.appendChild(tableContent);
		}
	});
}
...
if (document.readyState === 'loading') {
	...
	document.addEventListener(
		'DOMContentLoaded',
		display_fabricated_metal_pricing_table
	);
} else {
