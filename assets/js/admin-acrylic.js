function display_acryling_pricing_table() {
	...
	const acrylicAdminOptions = AcrylicAdmin.quote_options;

	const acrylicPricing = convert_json(
		acrylicAdminOptions.letter_height_x_logo_pricing
	);
	...
	logoThicknesses.forEach((thickness) => {
		...
		const table = acrylicAdminOptions.logo_pricing[pricingKey]
			? convertTableToJson(acrylicAdminOptions.logo_pricing[pricingKey])
			: false;
