...
class Woocommerce {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_order_get_total', array( $this, 'set_order_total_to_meta_value' ), 20, 2 );
		...
		add_action( 'woocommerce_save_product_variation', array( $this, 'save_multiple_items_field' ), 10, 2 );
		add_action( 'woocommerce_process_product_meta', array( $this, 'save_multiple_items_field_simple' ) );

		add_filter( 'woocommerce_sale_flash', array( $this, 'clearance_sale_flash' ), 10, 3 );

		// Add hooks for multiple items pricing
		add_action( 'woocommerce_product_options_pricing', [ $this, 'multiple_items_field_simple' ] );
		add_action( 'woocommerce_process_product_meta', [ $this, 'save_multiple_items_field_simple' ] );
		add_action( 'woocommerce_variation_options_pricing', [ $this, 'multiple_items_field' ], 10, 3 );
		add_action( 'woocommerce_save_product_variation', [ $this, 'save_multiple_items_field' ], 10, 2 );
		...
		// Product Comments Functionality
		add_action( 'woocommerce_product_options_general_product_data', array( $this, 'add_comments_option_to_products' ) );
		add_action( 'woocommerce_process_product_meta', array( $this, 'save_comments_option_for_product' ) );
		add_action( 'woocommerce_after_add_to_cart_button', array( $this, 'add_custom_product_field' ), 30 );
...
<?php
	...
	public function apply_multiple_items_price_filter( $price, $product ) {
		// Get cart item quantity if product is in cart
		$quantity = 0;
		if ( WC()->cart ) {
		...
		// Get threshold and special price from product meta
		$threshold = get_post_meta( $product->get_id(), 'multiple_items_threshold', true );
		$special_price = get_post_meta( $product->get_id(), 'multiple_items_price', true );

		// Apply special price if conditions are met
		if ( $threshold && $special_price && $quantity >= intval( $threshold ) ) {
			$currency = get_woocommerce_currency();
			$special_price = $currency === 'CAD' ?
				floatval( $special_price ) * NOVA_EXCHANGE_RATE :
				floatval( $special_price );
	...
	public function apply_multiple_items_price( $cart ) {
		...
		foreach ( $cart->get_cart() as $cart_item ) {
			// Get product ID (works for both simple and variable products)
	...
	public function save_multiple_items_field( $variation_id, $loop ) {
		$threshold = isset( $_POST['multiple_items_threshold'][ $loop ] ) ? absint( $_POST['multiple_items_threshold'][ $loop ] ) : '';
		$price = isset( $_POST['multiple_items_price'][ $loop ] ) ? wc_format_decimal( $_POST['multiple_items_price'][ $loop ] ) : '';

		if ( $threshold ) {
			update_post_meta( $variation_id, 'multiple_items_threshold', $threshold );
		} else {
			delete_post_meta( $variation_id, 'multiple_items_threshold' );
		}

		if ( $price ) {
			update_post_meta( $variation_id, 'multiple_items_price', $price );
		} else {
			delete_post_meta( $variation_id, 'multiple_items_price' );
		}
	}

	public function multiple_items_field_simple() {
		woocommerce_wp_text_input(
			array(
				'id' => 'multiple_items_threshold',
				'label' => 'Multiple Items Threshold',
				'data_type' => 'number',
				'desc_tip' => true,
				'description' => __( 'Enter the quantity threshold for special pricing', 'woocommerce' ),
			)
		);

		woocommerce_wp_text_input(
			array(
				'id' => 'multiple_items_price',
				'label' => 'Multiple Items Price',
				'data_type' => 'price',
				'desc_tip' => true,
				'description' => __( 'Enter the special price when threshold is met', 'woocommerce' ),
			)
		);
	}

	public function save_multiple_items_field_simple( $post_id ) {
		$threshold = isset( $_POST['multiple_items_threshold'] ) ? absint( $_POST['multiple_items_threshold'] ) : '';
		$price = isset( $_POST['multiple_items_price'] ) ? wc_format_decimal( $_POST['multiple_items_price'] ) : '';

		if ( $threshold ) {
			update_post_meta( $post_id, 'multiple_items_threshold', $threshold );
		} else {
			delete_post_meta( $post_id, 'multiple_items_threshold' );
		}

		if ( $price ) {
			update_post_meta( $post_id, 'multiple_items_price', $price );
		} else {
			delete_post_meta( $post_id, 'multiple_items_price' );
		}
	}
	...
	public function modify_variation_price_html( $variation_data, $product, $variation ) {
		if ( is_product() ) {
			...
			$availability_html = $variation_data['availability_html'];
			if ( isset( $attributes['attribute_pa_variation'] ) && $attributes['attribute_pa_variation'] === '1-pc' ) {
				$price_html = $variation_data['price_html'];
				$variation_data['price_html'] = $price_html . ' /each';
			...
			if ( isset( $attributes['attribute_pa_variation'] ) && $attributes['attribute_pa_variation'] === '10-pcs' ) {
			...
			if ( $multiple_items_threshold && $multiple_items_price ) {
				$variation_data['availability_html'] = '<p class="clearance-notice stock">Orders of ' . $multiple_items_threshold . ' or more pieces starts at ' . wc_price( $multiple_items_price, array( 'currency' => $currency ) ) . '</p>' . $availability_html;
	...
	public function modify_price_html( $price, $product ) {
		if ( $product->is_type( 'simple' ) && is_product() ) {
			$threshold = get_post_meta( $product->get_id(), 'multiple_items_threshold', true );
			$special_price = get_post_meta( $product->get_id(), 'multiple_items_price', true );

			if ( $threshold && $special_price ) {
				$currency = get_woocommerce_currency();
				$special_price = $currency === 'CAD' ? floatval( $special_price ) * NOVA_EXCHANGE_RATE : floatval( $special_price );

				// Check if product is in cart with quantity meeting threshold
				foreach ( WC()->cart->get_cart() as $cart_item ) {
					if ( $cart_item['product_id'] === $product->get_id() && $cart_item['quantity'] >= $threshold ) {
						return '<p class="price">' . wc_price( $special_price, array( 'currency' => $currency ) ) . '</p>';
					}
				}

				// Show regular price with threshold notice
				return $price . '<p class="clearance-notice stock">Orders of ' . $threshold . ' or more pieces starts at ' .
					wc_price( $special_price, array( 'currency' => $currency ) ) . '</p>';
			}
		}
		return $price;
	}
	...
	public function clearance_sale_add_to_cart_validation( $passed, $product_id, $quantity ) {
		...
		if ( $variation_id ) {
			...
			if ( isset( $attributes['pa_variation'] ) && $attributes['pa_variation'] === '10-pcs' ) {
				// Get quantity in cart for this specific product variation
				$cart_quantity = 0;
				foreach ( WC()->cart->get_cart() as $cart_item ) {
				...
				// Check if quantity for this specific product exceeds 6
				if ( ( $cart_quantity + $quantity ) > 6 ) {
					wc_add_notice( sprintf(
						'Maximum quantity for this bulk pack (10pcs) item is 6 units. You already have %d in cart.',
						$cart_quantity
					), 'error' );
					return false;
				}
	...
	public function update_ga_tracked( $order_id ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			if ( $product && stripos( $product->get_name(), 'test' ) !== false ) { // Case-insensitive check
				$contains_test_product = true;
				break; // Stop checking after first match
			}
		}

		// If a "test" product is found OR the customer is an admin, update order meta
		if ( $contains_test_product || $customer_is_admin ) {
	...
	public function nova_calculated_total( $total, $cart ) {
		...
		$total_tax = $subtotal + round( $computed_tax, 2 );
		$total = $total_tax - $discount;
	...
	public function move_product_to_trash_after_order( $order_id ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			if ( $item instanceof \WC_Order_Item_Product ) {
				...
				// Only delete Nova Quote products
				if ( $product && $product->get_type() === 'nova_quote' ) {
...
<p>
  ...
  <input type="text" name="shipping_carrier" id="shipping_carrier" value="<?php echo esc_attr( $shipping_carrier ); ?>"
    style="width:100%;" />
</p>
<p>
...
<?php
	...
	public function debug_order_meta_content( $post ) {
		...
		foreach ( $items as $item ) {
			echo '<h4>Item ID: ' . $item->get_id() . '</h4>';
			echo '<pre>';
    ...
    <div id="productCat" class="p-dropdown-current overflow-hidden grow">
      <div class="p-drowpdown-wrap grow h-[55px] p-[10px] dropdown-trigger" data-open="productCat-list">
        <div id="productCatCurrent" class="flex grow gap-2 items-center">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="15" height="8" viewBox="0 0 15 8" fill="none" class="ml-auto">
...
<?php
	...
	public function woocommerce_thankyou_order_received_text( $text, $order ) {
		if ( ! $order ) {
			return $text; // Return the original text if the order is not available
		}

		// Get the meta values from the order
		$from_order_id = $order->get_meta( '_from_order_id' );
		$second_payment = $order->get_meta( 'second_payment' );
		$original_order_ids = $order->get_meta( '_original_order_ids' );

		// If any of the specified meta values are present, modify the thank you text
		if ( ! empty( $from_order_id ) || ! empty( $second_payment ) || ! empty( $original_order_ids ) ) {
			$text = __( 'Thank you for the payment', 'nova-b2b' );
		} else {
			$text = __( '', 'nova-b2b' );
		}
	...
	public function nova_checkout_order_created( $order ) {
		...
		foreach ( $items as $item_id => $item ) {
			...
			if ( ! empty( $signage ) && isset( $quote_id ) ) {
				...
				$dropbox_urls = get_field( 'dropbox_urls', $quote_id );
				if ( ! empty( $dropbox_urls ) ) {
					update_field( 'dropbox_urls', $dropbox_urls, $order->get_id() );
				}
			}
		}
...
<?php
	...
	/**
	 * Add product comments option to product data panel
	 */
	public function add_comments_option_to_products() {
		woocommerce_wp_checkbox( array(
			'id' => '_enable_product_comments',
			'label' => __( 'Enable Order Comments', 'nova-b2b' ),
			'description' => __( 'Enable this to allow customers to add comments to this product', 'nova-b2b' )
		) );
	}

	/**
	 * Save the product comments option
	 * 
	 * @param int $product_id Product ID
	 */
	public function save_comments_option_for_product( $product_id ) {
		$enable_comments = isset( $_POST['_enable_product_comments'] ) ? 'yes' : 'no';
		update_post_meta( $product_id, '_enable_product_comments', $enable_comments );
	}

	/**
	 * Add comments field to single product page if enabled
	 */
	public function add_custom_product_field() {
		$product_id = get_the_ID();
		$enable_comments = get_post_meta( $product_id, '_enable_product_comments', true );
		$placeholder = get_field( 'product_comment_placeholder', $product_id ) ? get_field( 'product_comment_placeholder', $product_id ) : 'Add any special notes or comments about this product.';

		if ( $enable_comments === 'yes' ) {
			echo '<div class="product-order-comments" style="margin: 20px 0; padding-top: 20px; clear: both;">';
			echo '<label for="product_order_comments">' . esc_html__( 'Comments', 'nova-b2b' ) . '</label>';
			echo '<textarea name="product_order_comments" id="product_order_comments" rows="3" style="width: 100%; margin-top: 5px;" placeholder="' . esc_attr__( $placeholder, 'nova-b2b' ) . '"></textarea>';
			echo '</div>';
		}
	}

	/**
	 * Save product comments to cart item data
	 * 
	 ...
	 */
	public function add_product_comments_to_cart_item( $cart_item_data, $product_id, $variation_id ) {
		$enable_comments = get_post_meta( $product_id, '_enable_product_comments', true );

		if ( $enable_comments === 'yes' && isset( $_POST['product_order_comments'] ) && ! empty( $_POST['product_order_comments'] ) ) {
			$cart_item_data['product_comments'] = sanitize_textarea_field( $_POST['product_order_comments'] );
		}
		return $cart_item_data;
	}

	/**
	 * Display product comments in cart
	 * 
	 * @param array $item_data Item data
	 * @param array $cart_item Cart item
	 * @return array Modified item data
	 */
	public function display_product_comments_in_cart( $item_data, $cart_item ) {
		if ( isset( $cart_item['product_comments'] ) && ! empty( $cart_item['product_comments'] ) ) {
			$item_data[] = array(
				'name' => __( 'Comments', 'nova-b2b' ),
				'value' => wp_kses_post( $cart_item['product_comments'] )
			);
		}
		return $item_data;
	}
	...
	/**
	 * Transfer product comments to order items
	 * 
	 ...
	 */
	public function add_product_comments_to_order_items( $item, $cart_item_key, $values, $order ) {
		if ( isset( $values['product_comments'] ) && ! empty( $values['product_comments'] ) ) {
			$item->add_meta_data( __( 'Comments', 'nova-b2b' ), $values['product_comments'] );
		}
	}
