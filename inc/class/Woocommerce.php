...
class Woocommerce {
	...
	public function save_tracking_details( $order, $array ) {
		...
		error_log( "Order ID: $order_id, Tracking Number: $tracking_number, Carrier: $carrier, Ship Date: $ship_date" );
		...
		if ( $payment_order_id ) {
			...
			if ( $payment_order ) {
				...
				// Log if the payment order could not be retrieved
				error_log( "Payment order with ID $payment_order_id not found." );
			}
		} else {
			// Log if there is no payment order ID
			error_log( "No payment order ID found for order ID $order_id." );
		}
	}
