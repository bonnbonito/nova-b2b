...
class Woocommerce {
	...
	public function modify_variation_price_html( $variation_data, $product, $variation ) {
		if ( is_product() ) {
			...
			if ( $attributes['attribute_pa_variation'] === '10-pcs' ) {
				...
				$variation_data['availability_html'] = '<p class="clearance-notice stock">6pc maximum for sale bulk</p>' . $availability_html;
			}
		}
	...
	public function clearance_sale_add_to_cart_validation( $passed, $product_id, $quantity ) {
		...
		if ( $variation_id ) {
			...
			if ( isset( $attributes['pa_variation'] ) && $attributes['pa_variation'] === '10-pcs' && $quantity > 6 ) {
				wc_add_notice( 'Maximum quantity for bulk pack (10pcs) is 6 units', 'error' );
				return false;
			}
		}
