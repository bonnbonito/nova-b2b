...
class Nova_Quote {
	...
	public function __construct() {
		...
		add_action( 'save_post', array( $this, 'insert_quote_to_partner_meta' ), 10, 3 );
	...
	public function nova_quote_who_updated( $post ) {
		...
		$post_id = $post->ID;
		$quoted_by = get_post_meta( $post_id, 'quoted_by', true );
		$user_info = get_userdata( $quoted_by );
		echo '<h3 style="margin-bottom: 0;">Quoted by:</h3>';
		echo '<p style="margin-top: 0;"><strong>' . esc_html( $user_info->first_name ) . '</strong> (' . $user_info->user_email . ')</p>';
	}
	...
	public function custom_rewrite_rule() {
		add_rewrite_rule(
			'^custom/(?!custom-sculpture/)([^/]*)/([^/]*)/(installation|tech-specs|faqs|sample-board|quote|overview)/?$',
			'index.php?signage=$matches[1]/$matches[2]&pagetab=$matches[3]',
			'top'
		);

		add_rewrite_rule(
			'^custom/custom-sculpture/(installation|tech-specs|faqs|sample-board|quote|overview)/?$',
			'index.php?signage=custom-sculpture&pagetab=$matches[1]',
			'top'
		);
	}
	...
	public function create_nova_quote_product( $post_id ) {
		...
		update_post_meta( $post_id, 'date_quoted', current_time( 'Y-m-d H:i:s' ) );
		update_post_meta( $post_id, 'quoted_by', get_current_user_id() );
	...
	public function insert_quote_to_partner_meta( $post_id, $post, $update ) {
		if ( wp_is_post_autosave( $post_id ) || wp_is_post_revision( $post_id ) ) {
			return;
		}
		...
		if ( $quote_status['value'] !== 'ready' ) {
			delete_post_meta( $post_id, 'quoted_by' );
		}
			...
			<tr>
				<td style="padding-top: 20px; padding-bottom: 20px;">
					...
					foreach ( $signage as $project ) {
						...
						if ( $currency === 'CAD' ) {
							$price = number_format( floatval( $projectArray['usdPrice'] ) * NOVA_EXCHANGE_RATE, 2, '.', '' );
						}
	...
	public function quotes_nova_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'upload_rest' => esc_url_raw( rest_url( NOVA_REST_ROUTE_PREFIX . '/upload-quote-file' ) ),
				...
				'letters_pricing_api' => rest_url() . NOVA_REST_ROUTE_PREFIX . '/pricingletters/',
				'letters_multi_pricing_api' => rest_url() . NOVA_REST_ROUTE_PREFIX . '/multipricingletters/',
				'logo_pricing_api' => rest_url() . NOVA_REST_ROUTE_PREFIX . '/pricinglogos/',
				'quantity_discount_api' => rest_url() . NOVA_REST_ROUTE_PREFIX . '/quantity-discount/',
		...
		if ( ( is_product() || is_account_page() || is_page( 'custom' ) || is_page( 'custom-project' ) ) && is_user_logged_in() || get_post_type() === 'signage' ) {
	...
	public function add_other_email_meta_box() {
		add_meta_box(
			'nova_other_email_meta_box',
			'Other Email',
			array( $this, 'display_other_email_meta_box' ),
			'nova_quote',
			'side',
			'high'
		);
	...
	public function display_other_email_meta_box( $post ) {
		$other_email = get_post_meta( $post->ID, 'other_email', true );
		wp_nonce_field( 'nova_other_email_meta', 'nova_other_email_meta_nonce' );
		echo '<div class="other-email-meta-box">';
		echo '<p>Another email to also receive updates on this project</p>';
		echo '<p><label for="other_email"><strong>Email:</strong></label></p>';
		echo '<p><input type="email" id="other_email" name="other_email" value="' . esc_attr( $other_email ) . '" class="widefat"></p>';
		echo '</div>';
	}

	public function save_other_email_meta( $post_id ) {
		if ( ! isset( $_POST['nova_other_email_meta_nonce'] ) ||
			! wp_verify_nonce( $_POST['nova_other_email_meta_nonce'], 'nova_other_email_meta' ) ) {
			return;
		}
		...
		if ( isset( $_POST['other_email'] ) ) {
			update_post_meta( $post_id, 'other_email', sanitize_email( $_POST['other_email'] ) );
		}
	}
}