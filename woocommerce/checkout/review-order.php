...
<table class="shop_table woocommerce-checkout-review-order-table">
    ...
    <tfoot>
        ...
        <?php $payment_select = WC()->session->get( 'payment_select' ) ? WC()->session->get( 'payment_select' ) : 'full'; ?>
        <tr class="payment-selection">
            <th class="px-4 py-8">Payment type</th>
            <td colspan="2" class="px-4 py-8">
                <ul id="payment-select">
                    <li>
                        <label for="payment_full" class="block justify-end p-3 border rounded-md w-full max-w-sm">
                            <input type="radio" name="payment_select" value="full"
                                <?php echo ( $payment_select == 'full' ? 'checked' : '' ); ?> id="payment_full"
                                onchange="updateCheckoutTotal('full')">
                            <span>Full</span>
                            <span class="text-sm font-body block mt-2">Description</span>
                        </label>
                    </li>
                    <li>
                        <label for="payment_net30" class="block justify-end p-3 border rounded-md w-full max-w-sm">
                            <input type="radio" name="payment_select" value="net30" id="payment_net30"
                                <?php echo ( $payment_select == 'net30' ? 'checked' : '' ); ?>
                                onchange="updateCheckoutTotal('net30')">
                            <span>NET 310</span>
                            <span class="text-sm font-body block mt-2">Payment 30 Days after shipping</span>
                        </label>
                    </li>
                </ul>
            </td>
        ...
        <tr class="order-total future-payment">
            ...
            <td colspan="2" class="text-right px-4 py-8">
                <h5 class="uppercase text-3xl"><?php echo wc_price( WC()->session->get( 'original_total' ) ); ?></h5>
            </td>
        </tr>
        <?php endif; ?>
