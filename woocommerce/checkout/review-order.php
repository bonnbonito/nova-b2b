...
<table class="shop_table woocommerce-checkout-review-order-table">
				...
				<tr
					...
					<td class="product-name px-4 py-8" colspan="2">
						...
						if ( 'nova_quote' !== $_product->get_type() ) {
							...
							$quote_date = get_the_date( 'M d, Y', $quoteID );
							?>
							<div title="View Details" onclick='Fancybox.show([{ src: "#quote-<?php echo $quoteID; ?>", type: "clone" }]);'
							...
							<div class="text-xs uppercase">
								...
								<div>
									<span class="text-sm font-title tracking-[1.4px]">DATE:</span> <?php echo $quote_date; ?>
								</div>
								<div>
	...
	<tfoot>
		...
		<?php
		if ( 0 != WC()->session->get( 'deposit_chosen' ) ) :
			?>
			<tr class="order-total future-payment">
				<th class="px-4 py-8">
					<h2 class="uppercase text-3xl"><?php esc_html_e( 'Pending Payment', 'woocommerce' ); ?></h2>
				</th>
				<td colspan="2" class="text-right px-4 py-8">
					<h5 class="uppercase text-3xl">
						<?php echo wc_price( WC()->session->get( 'pending_amount' ), array( 'currency' => get_woocommerce_currency() ) ); ?>
					</h5>
				</td>
			</tr>
		<?php endif; ?>
